                                  ...
                                  std::set<ISelectableObject*>& selectedObjects) const
{
    ...
    raySceneQuery->setQueryMask(static_cast<short>(mQueryType));
    ...
    if (itr != result.end())
    {
        if (itr->movable)
        {
            ...
            auto tank = static_cast<Tank*>(itr->movable->getParentSceneNode()->getParentSceneNode());
            auto selectedObject = static_cast<ISelectableObject*>(tank);
                                   ...
                                   std::set<ISelectableObject*>& selectedObjects) const
{
    ...
    float left = first.x, right = second.x;
    float top = first.y, bottom = second.y;
    volQuery->setQueryMask(static_cast<short>(mQueryType));
    ...
    {
        auto tank = static_cast<Tank*>((*it)->getParentSceneNode()->getParentSceneNode());
        auto selectedObject = static_cast<ISelectableObject*>(tank);
        //selectedObject->showBoundingBox(true);
        selectedObject->ShowDecal();
        selectedObjects.insert(selectedObject);
...
std::set<ISelectableObject*> ObjectSelector::GetSelections() const
{
    return mSelectedObjects;
}

void ObjectSelector::SetQueryMask(QueryTypes queryType)
{
